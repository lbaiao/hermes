...
%   This is a protected method of LteInnerReceiver that returns the channel
...
switch ( this.channelEstimationAlgo )
    case enum.modem.ChannelEstimation.PERFECT
        ...
        % define beginning of OFDM symbols
        firstSymbolDuration = ( fftSize + ...
                                this.demodulator.cyclicPrefixLengthFirstSymbol ) / ...
                                this.demodulator.samplingRate;
        symbolDuration = ( fftSize + this.demodulator.cyclicPrefixLength ) / ...
                           this.demodulator.samplingRate;
        symbolsPerSlot = this.demodulator.ofdmSymbolsPerSlot;
        slotDuration = firstSymbolDuration + ( symbolsPerSlot - 1 ) * ...
                       symbolDuration;
        numberOfSlots = this.demodulator.numberOfSlots;
        
        % define instant for each symbol
        instantsInSlot = [ 0; firstSymbolDuration; ...
                           firstSymbolDuration + ...
                           ( 1:( symbolsPerSlot - 2 ) ).' * symbolDuration ];     
        
 
        samplingInstants = repmat( instantsInSlot, 1, numberOfSlots, 1 );
        slotBegins = repmat( ( 0 : numberOfSlots - 1 ) * slotDuration , ...
                             symbolsPerSlot, 1 );
        samplingInstants = samplingInstants + slotBegins + ...
                           samplingInstantsIn(1);
        samplingInstants = reshape( samplingInstants, 1, ...
                                    numberOfSlots * symbolsPerSlot );

        
        % interpolate
        nRxAnts = size( sampledChannel, 3 );
        nTxAnts = size( sampledChannel, 4 );
        nInstants = length( samplingInstants );
                ...
                if size( sampledChannel, 1 ) > 1
                    channel( :, :, rxAnt, txAnt ) = ...
                        interp1 ( samplingInstantsIn, ...
                            ...
                            samplingInstants, 'pchip', 'extrap' ).';
                else
                    channel(:, :, rxAnt, txAnt ) = ...
                        sampledChannel( 1, :, rxAnt, txAnt );
                end
            end
        end
