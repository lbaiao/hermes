function plotResults( resultsDir )
...
lowMargin = statistics.berMean .* (1 - statistics.berErrorMargin/2);
highMargin = statistics.berMean .* (1 + statistics.berErrorMargin/2);
confInterval = semilogy(statistics.snrVector, lowMargin, 'r:', ...
    statistics.snrVector, highMargin, 'r:');
...
lowMargin = statistics.blerMean .* (1 - statistics.blerErrorMargin/2);
highMargin = statistics.blerMean .* (1 + statistics.blerErrorMargin/2);
confInterval = semilogy(statistics.snrVector, lowMargin, 'r:', ...
    statistics.snrVector, highMargin, 'r:');
...
title(titleLabel);
legendLabel = [ 'HERMES V1.0 ' MODULATION ' ' WAVEFORM ', Block Size:' BLOCK_SIZE];
